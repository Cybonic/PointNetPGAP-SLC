...
from ..aggregators import multihead
from ..aggregators import MoE
from ..backbones.pointnet import *
from ..backbones import resnet
from networks.utils import *

class PointNetORCHNet(nn.Module):
    def __init__(self,aggregator="MultiHead",in_dim=3, feat_dim = 1024, num_points=2500, use_tnet=False, output_dim=1024):
        super(PointNetORCHNet, self).__init__()

        self.point_net = PointNet_features(dim_k=feat_dim,use_tnet = use_tnet, scale=1)
        
        if aggregator.startswith("MultiHead"):
            self.head = multihead.__dict__[aggregator](outdim=output_dim)
        elif aggregator.startswith("MoE"):
            self.head = MoE.__dict__[aggregator](outdim=output_dim)
